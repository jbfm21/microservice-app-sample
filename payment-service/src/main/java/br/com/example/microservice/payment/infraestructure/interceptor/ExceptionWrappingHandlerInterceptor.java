...
import br.com.example.microservice.payment.domain.exception.BusinessError;
import br.com.example.microservice.payment.domain.exception.BusinessErrorCode;
...
public class ExceptionWrappingHandlerInterceptor implements MessageHandlerInterceptor<CommandMessage<?>> 
{
    ...
    private BusinessError exceptionDetails(Throwable throwable) 
    {
        ...
        } if (throwable instanceof AggregateNotFoundException)
        {
        	...
        	BusinessError businessError = new BusinessError(throwable.getClass().getName(), BusinessErrorCode.PAYMENT_NOT_FOUND, String.format("Payment not found"));
