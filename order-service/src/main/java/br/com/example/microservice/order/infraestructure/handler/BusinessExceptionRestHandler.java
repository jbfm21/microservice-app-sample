package br.com.example.microservice.order.infraestructure.handler;
    ...
    public ResponseEntity<ApiError> handleCommandExecutionException(final CommandExecutionException ex, final WebRequest request) 
    {
        return ex.getDetails()
           .map((Object it) -> 
           {
               	...
               	log.error("Unable to process order: {}", businessError, ex);
           ...
           .orElseGet(() -> {
               	log.error("Unable to process order due to {}", ex.getMessage(), ex);
